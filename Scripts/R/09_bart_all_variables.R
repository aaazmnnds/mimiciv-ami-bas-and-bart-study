...
get_bart_importance <- function(data, y_col) {
  ...
  bart_model <- tryCatch({
    ...
    cat(sprintf("    [TIMEOUT/ERROR in get_bart_importance: %s]\n", conditionMessage(e)))
    return(NULL)
  })
...
evaluate_top_vars_bart <- function(train_data, test_data, y_col, top_vars) {
  total_vars <- length(top_vars)
  eval_indices <- unique(round(seq(1, total_vars, length.out = 10)))
  
  log_probs <- rep(NA, total_vars)
  all_k_preds <- vector("list", total_vars)
    ...
    for (method in METHODS) {
      for (use_mi in c(FALSE, TRUE)) {
        ...
        if (method == "MICE") {
          ...
          final_file_check <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_POOLED_predictions", output_suffix, ".csv")
        } else {
          files_indices <- c(1)
          output_suffix <- "_m1"
          final_file_check <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_predictions", output_suffix, ".csv")
        }
        ...
        for (fold in 1:NUM_OF_FOLDS) {
          ...
          for (imp_idx in files_indices) {
            ...
            if (is.null(ranked_vars)) next
            top_k_vars <- ranked_vars[1:length(ranked_vars)]
        ...
        if (length(all_logprobs) > 0) {
           ...
           if (method == "MICE") {
               ...
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_POOLED_log_probabilities", output_suffix, ".csv")
           } else {
               df_log <- df_log %>% group_by(num_top) %>% summarise(avg_log_prob = mean(log_prob, na.rm=TRUE), .groups="drop")
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_log_probabilities", output_suffix, ".csv")
           }
        ...
        if (length(all_selected) > 0) {
           ...
           if (method == "MICE") {
               ...
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_POOLED_selected_variables", output_suffix, ".csv")
           } else {
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_selected_variables", output_suffix, ".csv")
           }
        ...
        if (length(all_preds) > 0) {
           ...
           if (method == "MICE") {
               ...
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_POOLED_predictions", output_suffix, ".csv")
           } else {
               fname <- paste0("Results/CORRECTED_ALL_VARS/results_BART_", full_name, "_predictions", output_suffix, ".csv")
           }
