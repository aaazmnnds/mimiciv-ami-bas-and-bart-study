...
configs <- list(
  ...
  list(name = "MIMIC_MICE", method = "MICE", 
       train_pattern = "Data/%d_90train_datamimiciii_mice_imputed_%d.csv", 
  ...
  list(name = "MI_MICE", method = "MICE", 
       train_pattern = "Data/%d_90train_dataMI1_%d.csv", 
  ...
  for (cfg in configs) {
    ...
    for (m in m_list) {
       ...
       if (length(all_folds_log_probs) > 0) {
           ...
           fname <- paste0("Results/results_BART_", cfg$name, output_suffix, ".csv")
