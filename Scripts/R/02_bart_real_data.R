      ...
      for (use_mi in c(FALSE, TRUE)) {
        ...
        for (fold in 1:NUM_OF_FOLDS) {
          ...
          for (imp_idx in files_indices) {
            ...
            all_preds[[length(all_preds)+1]] <- data.frame(fold = fold, imp_idx = imp_idx, true_label = test_aug[[cfg$y_col]], predicted_prob = res$preds_best, num_top = res$best_k)
          }
          cat(" Done\n")
        }
        ...
        if (length(all_preds) > 0) {
           ...
           if (method == "MICE") {
               df_pred <- df_pred %>% group_by(fold, true_label, num_top) %>% summarise(predicted_prob_pooled = mean(predicted_prob, na.rm=TRUE), .groups="drop")
